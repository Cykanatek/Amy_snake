...
class Clanek(object):
    ...
    def zmen_barvu(self, barva):
        ...
        if (self.id != None): # v přápadě, že článek byl vykreslen
    ...
    def je_nademnou(self, clanek):
        ...
        if clanek.id in self.canvas.find_all():  ### zjistí zda je clánek zobrazen na canvasu
            ...
            ### následující zjistí zda se zobrazené jádlo (identifikátor mé instance jídla na canvasu) 
    ...
    def krok_vpred1(self):
        ...
        for jidlo in self.jidla: ### projdeme všechna zobrazená jídla
            ...
            if jidlo.je_nademnou(self.housenka1.get_hlava()) and jidlo.barva == "red":
                ...
                ### podmínka pro limit po sežrání věech jídel - nevím, proč nefunguje, už to běželo, jen jsem něco provedla a nějak pokazilo to samé pak na řádku 360-362
                if len(sezrane) == 5:
                    self.destroy()
                    self.ahoj()

        for clanek in self.housenka1.clanky: ## da je lava na clanku
            if clanek.je_nademnou_(self.housenka1.get_hlava()) == True:
                self.destroy()
